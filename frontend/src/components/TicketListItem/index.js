...
const TicketListItem = ({ ticket, filteredTags }) => {
  ...
  useEffect(() => {
        ...
        if (data.ticket && data.ticket.id === ticket.id) {
          setCurrentTicket((prevTicket) => {
            const updated = {
              ...
              unreadMessages:
                data.ticket.unreadMessages !== undefined
                  ? data.ticket.unreadMessages
                  : prevTicket.unreadMessages,
            };
    ...
    const handleContactUpdate = (data) => {
      if (data.action === "update" && data.contact) {
        if (
          data.contact.id === ticket.contactId ||
          data.contact.id === ticket.contact?.id
        ) {
  ...
  const handleAcceptTicket = async (id) => {
    ...
    try {
      ...
      const isGroup =
        ticket?.contact?.isGroup || currentTicket?.contact?.isGroup;
  ...
  const canTabsSettings = (ts) => {
    return (
      (settings &&
        settings.some((s) => s.key === ts && s.value === "enabled")) ||
          ...
          primary={
            <ContactName>
              ...
              {currentTicket.pinnedChat && (
                <Tooltip
                  title={t("chatActions.pinnedIndicator")}
                  arrow
                  placement="top"
                >
                  <PushPin
              ...
              {currentTicket.mutedChat && (
                <Tooltip
                  title={t("chatActions.mutedIndicator")}
                  arrow
                  placement="top"
                >
                  <VolumeOff
              ...
              {currentTicket.favoritedChat && (
                <Tooltip
                  title={t("ticketListMenu.favoritedIndicator")}
                  arrow
                  placement="top"
                >
                  <Favorite
          ...
          secondary={
            <Box sx={{ display: "flex", flexDirection: "column", gap: "2px" }}>
                    ...
                    {(() => {
                      if (currentTicket.lastMessage) {
                        if (currentTicket.lastMessage.includes("🢅") === true) {
                          ...
                          currentTicket.lastMessage.includes("🢇") === true
                        ) {
                          return (
                            <img
                              src={receiveIcon}
                    ...
                    {currentTicket.lastMessage && (
                      ...
                      >
                        {currentTicket.lastMessage
                          ...
                          .replace("🢇", "")
                          .replace("🢅", "") +
                          (currentTicket.lastMessage.length > 40 ? "..." : "")
                            .replace("🢇", "")
                            .replace("🢅", "")}
                      </WhatsMarked>
                    )}
              ...
              >
                {ticket.whatsappId && (
                  <Tooltip
                    title={t("ticketsList.items.connection")}
                    placement="bottom"
                    arrow
                  >
                    <ChipRadiusDot
                      sx={{
                        backgroundColor:
                          ticket.whatsapp?.color || theme.palette.grey[300],
                        fontSize: "0.6em",
                        fontWeight: "bold",
                        height: 12,
                        padding: "2px 0px",
                      ...
                      label={(
                        ticket.whatsapp?.name || t("ticketsList.items.user")
                      ).toUpperCase()}
                    />
                  </Tooltip>
                )}
                {ticket.status !== "pending" && ticket?.user?.name && (
                  <Tooltip
                    title={t("ticketsList.items.user")}
                    placement="bottom"
                    arrow
                  >
                    <ChipRadiusDot
        ...
        <ButtonContainer>
          {ticket.status === "pending" &&
            (ticket.queue === null || ticket.queue === undefined) && (
              <Tooltip
                title={t("ticketsList.items.accept")}
                placement="bottom"
                arrow
              >
                <BottomButton
          ...
          {ticket.status === "pending" && (
            <Tooltip
              title={t("ticketsList.items.close")}
              placement="bottom"
              arrow
            >
              <BottomButton
          ...
          {canTabsSettings("tabsPending") && ticket.status === "open" && (
            <Tooltip
              title={t("ticketsList.items.return")}
              placement="bottom"
              arrow
            >
              <BottomButton
